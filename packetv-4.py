...
# Combine all traffic, intersperse normal traffic among attack traffic
all_traffic = []
all_attack_traffic = (
    attack_traffic_same_dst_ip + 
    attack_traffic_same_src_ip + 
    attack_traffic_same_pkt_size + 
    attack_traffic_same_flow_time
)
normal_index = 0
normal_interval = len(all_attack_traffic) // len(normal_traffic)

for i, attack_packet in enumerate(all_attack_traffic):
    all_traffic.append(attack_packet)
    if (i + 1) % normal_interval == 0 and normal_index < len(normal_traffic):
        all_traffic.append(normal_traffic[normal_index])
        normal_index += 1
...
# Write to CSV file
with open("generated_traffic2.csv", "w", newline='') as csvfile:
