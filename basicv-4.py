...
# Read the dataset
data = pd.read_csv('SDN-dataset.csv')
...
def reset_threshold(entropy_values):
    # Use recent normal traffic to calculate a new threshold
    recent_normal_traffic = entropy_values[-200:]  # Last 200 windows assumed to be recent normal traffic
    if len(recent_normal_traffic) < window_size:
        recent_normal_traffic = entropy_values[:window_size]
    recent_mean = np.mean(recent_normal_traffic)
    recent_ci = 1.96 * np.std(recent_normal_traffic) / np.sqrt(len(recent_normal_traffic))
    return recent_mean - recent_ci
...
for index, row in features.iterrows():
    if packet_count % window_size == 0 and packet_count != 0:
        ...
        if H < theta:
            if attack_count == 0:
                pre_attack_threshold = theta  # Save the threshold before the first attack
            ...
            if consecutive_anomalies >= 5:
                ...
                if attack_count >= 3:
                    # Reset the threshold after three attacks
                    theta = reset_threshold(entropy_values)
                    attack_count = 0
                    print("Threshold reset to recent normal traffic value.")
        else:
            consecutive_anomalies = 0
